...
var() enum ETriggerType
{
	...
	TT_LivePlayerProximity,		// Trigger only activated by live player, untriggered if player dies while touching
} TriggerType;
...
{
	if ( bInitiallyActive && (TriggerType == TT_LivePlayerProximity) )
	...
	switch( TriggerType )
	{
		...
		case TT_ClassProximity:
			return ClassIsChildOf(Other.Class, ClassProximityType) 
				&& ( !ClassIsChildOf(ClassProximityType, class'Vehicle') || Pawn(Inst).IsPlayerPawn() );
				// Force Pawns to be controlled
